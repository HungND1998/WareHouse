...
export default function StockIn() {
  ...
  function load() {
    setLoading(true);
    api.get('/stock-in').then((res) => setRows(res.data)).catch((err) => push(err.message, 'error')).finally(() => setLoading(false));
  }
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!warehouseId) return push('Vui lòng chọn kho nhập.', 'error');
    ...
    setSaving(true);
    try {
      await api.post('/stock-in', { warehouse_id: Number(warehouseId), supplier_id: supplierId ? Number(supplierId) : null, note, items: cleanItems });
      push('Đã tạo phiếu nhập kho.');
      setModalOpen(false);
      load();
    } catch (err) {
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="eyebrow">Nhập hàng vào kho</div>
          ...
          <table className="data-table">
            ...
            <tbody>
              {rows.length === 0 && !loading && (
                <tr className="empty-row"><td colSpan={6}>Chưa có phiếu nhập nào.</td></tr>
              )}
              {rows.map((r) => (
      ...
      {detailOpen && (
        <Modal title={`Phiếu nhập ${detailOpen.code}`} onClose={() => setDetailOpen(null)}>
